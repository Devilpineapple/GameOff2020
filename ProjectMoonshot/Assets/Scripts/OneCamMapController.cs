...
public class OneCamMapController : MonoBehaviour
{
    ...
    private void Awake()
    {
        cam.transform.localPosition = new Vector3(settings.mapData.dimension.width / 2f, settings.mapData.dimension.height, -(settings.mapData.dimension.height / 2f) - 0.5f);
